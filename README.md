...
- Six tabs (five reports + a litter planner):
  ...
  - **DNA Tests** — a **whole-database** view of one recessive genetic test, chosen from a dropdown (Ataxia `SAMS-KCNJ10` · PRA-rcd4 `PRA-rcd4-C2orf71` · Wilson `WD-ATP7B` · MDR2 `MDR2-ABCB1` · Factor VII `F7` · CURV `CUR/N`). Shows a genotype pie chart — Clear / Carrier / Affected, with counts and percentages — over the list of every dog that has a result: **Pedigree No.** (the record number in the database) · registered name · genotype · sire · dam · date of birth. Genotypes are shown exactly as the database records them; the classification behind the chart reads the common lab spellings (`CLEAR (WT/WT)`, `N/N`, `WT/MUT`, `heterozygous`, …) and anything it cannot place is listed under "Other / unclear" rather than dropped. Untested dogs are not listed. Exports to **PDF** and **CSV**. This is the one report that needs no selected dog.
...
- **Export** any report with **Save…**: a single-page **PDF** (A4, or A3 landscape for a deep chart); for the bracket chart, a full-resolution **PNG** of the whole tree with no page limit; for the Indented Tree, a plain-text **.txt** file; for the DNA Tests report, a **.csv** spreadsheet (UTF-8 with a BOM, so accented kennel names open correctly in Excel).
...
- Recessive DNA-test result columns (text): `SAMS-KCNJ10`, `PRA-rcd4-C2orf71`, `WD-ATP7B`, `MDR2-ABCB1`, `F7`, `CUR/N`. Each drives the **DNA Tests** report when present; `PRA-rcd4-C2orf71` and `SAMS-KCNJ10` are additionally used by Hypothetical Mating, which shows the sire's/dam's values on the projected chart and warns on carrier×carrier (or affected) risk. A column that is absent is simply reported as having no data — never an error.
...
- `src/lib/dnaReport.ts` — the DNA Tests report: the test catalogue (keyed to `sourceFields.ts`, so the SQL column is defined in one place), the genotype classifier, the report builder and the CSV serializer. Pure — no DB, no DOM.
...
- `src/App.tsx` is a **thin shell** (connection status, selected animal, active tab, toolbar depth selectors, export). Each report tab is a **self-contained view component** — `PedigreeView` · `IndentedTreeView` · `LinebreedingView` · `FoundationView` · `HypotheticalMatingView` · `DnaTestView` — that fetches its own data through the `src/hooks/useResource.ts` hook (loading/error + cancelled-guard) and reports readiness up. Adding a report is roughly one new `*View.tsx`, one `TABS` entry, and one render line; the hook lives in `src/hooks/` so `src/lib/` stays pure and main-process-safe.
